defmodule Exercism.NewPassport do
  def get_new_pasport(now, birthday, form), do:
    with({:ok, timestamp} <- enter_building(now),
        ...
        do: {:ok, get_new_passport_number(timestamp, counter, checksum)},
        else: ({:coffee_break, _} -> {:retry, NaiveDateTime.add(now, 15, :minute)}; error -> error))
