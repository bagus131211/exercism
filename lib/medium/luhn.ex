defmodule Exercism.Luhn do
  ...
  def valid?(number), do:
    number
      ...
      |> then(& cond do:
                (
                  Regex.match?(~r/[^[:digit:]]/, &1) -> false
                  String.length(&1) <= 1 -> false
                  true -> &1 |> check
                )
             )
  ...
  defp check(number), do:
    ...
    |> Enum.with_index
    |> Enum.map(fn {n, idx} ->
        if Integer.is_odd(idx), do:
          (case String.to_integer(n) * 2, do:
            (
              x when x > 9 -> x - 9
              x -> x
            )
          ),
        else: String.to_integer(n)
      end)
    |> Enum.sum()
    |> rem(10) == 0
end
